...
function highlight_subnav(url){
	$('.sub-menu').find('[href="'+url+'"]').closest('li').addClass('active');
	$('.sub-menu').find('[href="'+url+'"]').closest('li').parent().css("display","block");
	$('.sub-menu').find('[href="'+url+'"]').closest('li').parent().parent().addClass('open');
	$('.sub-menu').find('[href="'+url+'"]').closest('li').parent().parent().parent().css("display","block");
	$('.sub-menu').find('[href="'+url+'"]').closest('li').parent().parent().parent().parent().addClass('open');
	$('.sub-menu').find('[href="'+url+'"]').closest('li').parent().parent().parent().parent().addClass('active');
}
//导航高亮
function highlight_nav(url){
	$('#menu_tit_tab').find('[href="'+url+'"]').closest('li').addClass('on');
}
function highlight_this(url){
	$('.highlight_this').find('[href="'+url+'"]').parent().css("display","block");
}
function highlight_this_nav(Aarray,url){
	if(Aarray.indexOf(url)>-1){
		$('.highlight_this').find('[href*="'+url+'"]').parent().css("display","block");
	};
}
function  get_today_time(){
	...
	var m = d.getMinutes();
	var se = d.getSeconds();
	return addZeroToNum(vYear)+'-'+addZeroToNum(vMon)+'-'+addZeroToNum(vDay)+" "+addZeroToNum(h)+':'+addZeroToNum(m);
	//s=vYear+(vMon<10 ? "0" + vMon : vMon)+(vDay<10 ? "0"+ vDay : vDay)+(h<10 ? "0"+ h : h)+(m<10 ? "0" + m : m)+(se<10 ? "0" +se : se);
	//return s;
}
function addZeroToNum(number){
	number=(+number);
	var numberList=["00","01","02","03","04","05","06","07","08","09"],rs="";
	if(typeof number === "number" && number != NaN){
		if( number>=0 && number<10 ){
			rs = numberList[number];
		}else{
			rs=number.toString();
		}
	}
...
admin.remove= function(id){
	if(confirm('您确认要删除该条信息!')) {
		var url= '{:U("new_index.php?m=hr&c=absence&a=dellog")}';
		$.post(url,{id:id},function(res){
			var data_res=eval(res);
			if(data_res){
				alert("删除成功...");
